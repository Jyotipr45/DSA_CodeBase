...
public static void main(String args[]) {
    ...
    static String maxRepeatingWord(int N, ArrayList<String> arr) {
        ...
        //count freq of each word in the array
        for(String word : arr){
            //if the word is already in the map ,Increase its count by 1
            //otherwise , add the word with an initial count of 1
            freqMap.put(word, freqMap.getOrDefault(word, 0) + 1);
        }

        //variable to store the most frequent word
        String mostFreqWord = "";
        ...
        for(Map.Entry<String, Integer> entry : freqMap.entrySet()){
            String word = entry.getKey(); // get the word
            int count = entry.getValue(); // get the count of the word

            if(count > maxCount || (count == maxCount && word.compareTo(mostFreqWord) < 0)){
                maxCount = count;
                mostFreqWord = word;
            }
