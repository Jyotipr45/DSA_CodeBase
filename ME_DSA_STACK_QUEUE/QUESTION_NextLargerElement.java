...
class NextLargerElement {
	...
	static int [] nextLargerElement(int nums[], int n) {
		// Array to store the next larger element for each element
		...
		for (int i = 0; i < n; i++) {
			...
			while (!st.isEmpty() && current > nums[st.peek()]) {
				// Set the next larger element for the element at the top of the stack
				result[st.pop()] = current;
			}
			// Push the index of the current element onto the stack
			st.push(i);
		}

		// For elements in the stack without a next larger element, set the result to -1
		while (!st.isEmpty()) {
			result[st.pop()] = -1;
		}

		// Return the array containing the next larger element for each element
		return result;
	}
